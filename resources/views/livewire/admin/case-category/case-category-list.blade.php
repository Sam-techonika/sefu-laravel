    ...
    <div class="card">
        <div class="card-body p-0">
            <table class="table table-vcenter card-table table-hover mb-0">
                ...
                <tbody>
                    @forelse ($caseCategories as $caseCategory)
                        <tr>
                            <td class="text-center fw-bold">{{ $caseCategory->id }}</td>
                            <td>
                                @foreach ($caseCategory->translations as $t)
                                    <span class="badge bg-primary-lt me-1">
                                        {{ strtoupper($t->locale) }}: {{ $t->name }}
                                    </span>
                                @endforeach
                            </td>
                            <td class="text-center">
                                <button class="btn btn-sm btn-warning" wire:click="openModal({{ $caseCategory->id }})">
                                    <i class="ti ti-edit"></i>
                                </button>
                                <button class="btn btn-sm btn-danger" wire:click="confirmDelete({{ $caseCategory->id }})">
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="text-center text-muted py-3">No case categories found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
            ...
            <div class="modal-content">
                <form wire:submit.prevent="saveCategory">
                    <div class="modal-header">
                        <h5 class="modal-title">{{ $isEdit ? 'Edit Case Category' : 'Add Case Category' }}</h5>
